...
import bire_linearization as bire
...
trim_solution = trim.trim(V, H, gamma, phi, Gamma,
                          shss=False, bire=True,
                          cg_shift=cg_shift,
                          fixed_point=False,
                          compressible=False,
                          aero_dir=aero_dir)
with open('./BIRE_solution.trim', 'wb') as f:
    pickle.dump(trim_solution, f)
...
linearization = bire.LinearizationBIRE(props, aero_dir)
...
lin_results = bire.Lin_Results(len(A), len(B[0]))
lin_results.A = A
lin_results.B = B
lin_results.C = C
...
with open('./BIRE_linearization.lin', 'wb') as f:
    pickle.dump(lin_results, f)


figs, axs = simulation_results(True, cg_shift,  dt=0.1)
...
fig_V.tight_layout()
fig_V.savefig('./Control Development/bire_uvw.pdf', dpi=1000)

ylims = (-25., 25.)
dy = {'major': 10., 'minor': 10./4.}
ax_R1 = pretty_plot(ax_R1, xlims, ylims, dx, dy)
ax_R1.annotate(r'\boldmath$p$', (8, 5), fontsize=16)
...
ax_R2.annotate(r'\boldmath$q$', (14, 1), fontsize=16)
ax_R2.grid()
ax_R3 = pretty_plot(ax_R3, xlims, ylims, dx, dy)
ax_R3.annotate(r'\boldmath$r$', (14, 1), fontsize=16)
ax_R3.grid()
...
fig_R.tight_layout()
fig_R.savefig('./Control Development/bire_pqr.pdf', dpi=1000)
...
fig_X.tight_layout()
fig_X.savefig('./Control Development/bire_xyz.pdf', dpi=1000)

ylims = (-15, 15)
dy = {'major': 6., 'minor': 6./4.}
ax_phi = pretty_plot(ax_phi, xlims, ylims, dx, dy)
ax_phi.annotate(r'\boldmath$\phi$', (8, 3), fontsize=16)
...
ax_theta.annotate(r'\boldmath$\theta$', (14, 4), fontsize=16)
ax_theta.grid()
ylims = (-7, 3)
dy = {'major': 2, 'minor': 2/4.}
ax_psi = pretty_plot(ax_psi, xlims, ylims, dx, dy)
ax_psi.annotate(r'\boldmath$\psi$', (14, 1), fontsize=16)
ax_psi.grid()
...
fig_O.tight_layout()
fig_O.savefig('./Control Development/bire_orientation.pdf', dpi=1000)

ylims = (-75, 75)
dy = {'major': 30., 'minor': 30./4.}
ax_uz = pretty_plot(ax_uz, xlims, ylims, dx, dy)
ax_uz.annotate(r'\boldmath$\Delta u$', (8, 15), fontsize=16)
...
ax_vz.annotate(r'\boldmath$\Delta v$', (14, 15), fontsize=16)
ax_vz.grid()
ax_wz = pretty_plot(ax_wz, xlims, ylims, dx, dy)
ax_wz.annotate(r'\boldmath$\Delta w$', (14, 15), fontsize=16)
ax_wz.grid()
...
fig_Vz.tight_layout()
fig_Vz.savefig('./Control Development/bire_d_uvw.pdf', dpi=1000)

ylims = (-25, 25)
dy = {'major': 10., 'minor': 10./4.}
ax_pz = pretty_plot(ax_pz, xlims, ylims, dx, dy)
ax_pz.annotate(r'\boldmath$\Delta p$', (8, 5), fontsize=16)
...
ax_qz.annotate(r'\boldmath$\Delta q$', (14, 1), fontsize=16)
ax_qz.grid()
ax_rz = pretty_plot(ax_rz, xlims, ylims, dx, dy)
ax_rz.annotate(r'\boldmath$\Delta r$', (14, 1), fontsize=16)
ax_rz.grid()
...
fig_Rz.tight_layout()
fig_Rz.savefig('./Control Development/bire_d_pqr.pdf', dpi=1000)
...
ax_thetaz.annotate(r'\boldmath$\Delta \theta$', (14, 0.5), fontsize=16)
ax_thetaz.grid()
...
fig_Oz.tight_layout()
fig_Oz.savefig('./Control Development/bire_d_orientation.pdf', dpi=1000)

xlims2 = (-1, 21)
dx2 = {'major': 5., 'minor': 5/4.}
fig_u.supylabel(r'\textbf{Control Deflections [deg]}', fontsize=16)
...
ax_dr.annotate(r'\boldmath$\delta_B$', (7, 5), fontsize=16)
ax_dr.grid()
...
fig_u.tight_layout()
fig_u.savefig('./Control Development/bire_control.pdf', dpi=1000)

xlims2 = (-1, 21)
dx2 = {'major': 5., 'minor': 5/4.}
fig_udot.supylabel(r'\textbf{Control Rates [deg/s]}', fontsize=16)
...
ax_dedot.annotate(r'\boldmath$\dot{\delta_e}$', (7, 1), fontsize=16)
ax_dedot.grid()
ylims = (-5, 5)
dy = {'major': 2, 'minor': 2/4.}
ax_drdot = pretty_plot(ax_drdot, xlims2, ylims, dx2, dy)
ax_drdot.annotate(r'\boldmath$\dot{\delta_B}$', (7, 1), fontsize=16)
ax_drdot.grid()
...
fig_udot.savefig('./Control Development/bire_control_rates.pdf', dpi=1000)
...
ax_w.grid()
fig_w.tight_layout()
fig_w.savefig('./Control Development/bire_wind.pdf', dpi=1000)
...
# # ax.axhline(-3., color='0.5', linestyle=':')
# # max_mask = 20.*np.log10(np.absolute(svd_S_max)) < -3.
# # min_mask = 20.*np.log10(np.absolute(svd_S_min)) < -3.
# # ax.axvline(s[max_mask][0], 0, (90 - 3.)/180, color='0.5', linestyle=':')
# # ax.axvline(s[min_mask][0], 0, (90 - 3.)/180., color='0.5', linestyle=':')
# xlims = (s[0], s[-1])
# dx = {}
# ylims = (-115, 115)
# dy = {'major': 30, 'minor': 30/4}
...
# fig.tight_layout()
# fig.savefig('./Control Development/bire_svd.pdf', dpi=1000)
