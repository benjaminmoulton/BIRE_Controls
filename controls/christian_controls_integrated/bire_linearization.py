...
class LinearizationBIRE:
    def __init__(self, props, aero_dir=aero_directory, N=8, M=4):
        ...
        Ixx = I_model.Ixx_coeffs #["Ixx"]
        Iyy = I_model.Iyy_coeffs #["Iyy"]
        Izz = I_model.Izz_coeffs #["Izz"]
        Ixz = I_model.Ixz_coeffs #["Ixz"]
        Ixy = I_model.Ixy_coeffs #["Ixy"]
        self.Iyz_coeffs = I_model.Iyz_coeffs #["Iyz"]
    ...
    def dI_yz(self, dB):
        # I_model = json.load(open('./bire_inertia_model.json'))
        Iyz = self.Iyz_coeffs # I_model["Iyz"]
        if dB == 0.:
    ...
    def set_linearization_point(self, x_hat, u_hat, alpha_hat, beta_hat, FM_hat,
        ...
        # Tx = self.u_hat[0] * trim.thrust(self.V_hat, self.props)
        aero = BIREAero(self.aero_dir,thrust_dir=self.aero_dir)
        aero.evaluate_coeffs(self.dB_hat)
        aero.evaluate_derivatives(self.dB_hat)
        Tx = aero.get_thrust(self.u_hat[0],self.props.H,self.V_hat)
        FX = dim_const*CX + Tx
        FY = dim_const*CY
        FZ = dim_const*CZ
        self.Mx_hat = dim_const*self.b_w*self.Cl_hat - FZ*self.Dy + FY*self.Dz
        self.My_hat = dim_const*self.c_w*self.Cm_hat - FX*self.Dz + FZ*self.Dx
    ...
    def _I_inv(self, dB):
        ...
        I_inv[1, 2] = Ixx*Iyz + Ixy*Ixz
        I_inv[2, 0] = Iyz*Ixy + Iyy*Izx
    ...
    def _M1(self):
        ...
        Ixz = self.I_xz(self.dB_hat)
        Ixy = self.I_xy(self.dB_hat)
        M1 = (self.Mx_hat + np.matmul(self._H_matrix()[0],np.array([
                self.x_hat[3],self.x_hat[4],self.x_hat[5]
            ])) + 
    ...
    def _M2(self):
        ...
        Ixz = self.I_xz(self.dB_hat)
        Ixy = self.I_xy(self.dB_hat)
        M2 = (self.My_hat + np.matmul(self._H_matrix()[1],np.array([
                self.x_hat[3],self.x_hat[4],self.x_hat[5]
            ])) +
    ...
    def _M3(self):
        ...
        Ixz = self.I_xz(self.dB_hat)
        Ixy = self.I_xy(self.dB_hat)
        M3 = (self.Mz_hat + np.matmul(self._H_matrix()[2],np.array([
                self.x_hat[3],self.x_hat[4],self.x_hat[5]
            ])) +
...
if __name__ == "__main__":
    ...
    trim_solution = trim.trim(V, H, gamma, phi, Gamma, fixed_point=False,cg_shift=cg_shift,
                              aero_dir=aero_dir,compressible=True, stall=True, bire=True)
    print()
    x_hat = trim_solution.states
    alpha_hat = trim_solution.x[1]
    beta_hat = trim_solution.x[2]
