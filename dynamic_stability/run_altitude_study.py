...
altitude_array = np.array(np.linspace(1000,15000,10))
phi = 0.0
...
for i in range(len(altitude_array)):
    ...
    case.update_aircraft_properties(V, altitude_array[i], dB = 0.0)
    
    case.solve_equilibrium_state(V, altitude_array[i], gamma, phi, cg_shift)
...
if save_figs == True:
    plt.savefig(directory + 'phugoid_altitude.png', dpi=250)
...
if save_figs == True:
    plt.savefig(directory + 'short_period_altitude.png', dpi=250)
...
if save_figs == True:
    plt.savefig(directory + 'dutch_roll_altitude.png', dpi=250)
...
labelF = 'Baseline'
labelB = 'BIRE'
plt.scatter(altitude_array, real_list_f16[:,F16_SR_ind], marker=f16_marker, color='k', s = size,label=labelF)
plt.scatter(altitude_array, real_list_BIRE[:,BIRE_SR_ind], marker=BIRE_marker, color='k', s = size,label=labelB)
...
if save_figs == True:
    plt.savefig(directory + 'spiral_altitude.png', dpi=250) 
 
plt.figure(4, figsize=fig_size)
labelF = 'Baseline'
labelB = 'BIRE'
plt.scatter(altitude_array, real_list_f16[:,F16_RL_ind], marker=f16_marker, color='k', s = size,label=labelF)
plt.scatter(altitude_array, real_list_BIRE[:,BIRE_RL_ind], marker=BIRE_marker, color='k', s = size,label=labelB)
...
if save_figs == True:
    plt.savefig(directory + 'roll_altitude.png', dpi=250)
