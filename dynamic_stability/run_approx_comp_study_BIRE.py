...
# bank_angle_array = np.array(np.linspace(0.0,5.0,4))
...
for i in range(len(bank_angle_array)):
    ...
    case = dynamicAnalysis(path='./', write_output = False, output_filename = 'eig_vals_BIRE_60deg_bank_cg_shift.txt',
                            BIRE=True, shss=SHSS, compressible=COMP,
                            stall=STALL, simple_thrust=SIMPLE_THRUST, cg_shift=cg_shift)
...
for i in range(len(bank_angle_array)):
    ...
    case = dynamicAnalysis(path='./', write_output = False, output_filename = 'eig_vals_BIRE_60deg_bank_cg_shift.txt',
                            BIRE=True, shss=SHSS, compressible=COMP,
                            stall=STALL, coords_approx=False,derivs_approx=True,
                            simple_thrust=SIMPLE_THRUST,  cg_shift=cg_shift)
...
for i in range(len(bank_angle_array)):
    ...
    case = dynamicAnalysis(path='./', write_output = False, output_filename = 'eig_vals_BIRE_60deg_bank_cg_shift.txt',
                            BIRE=True, shss=SHSS, compressible=COMP,
                            stall=STALL, coords_approx=True,derivs_approx=False,
                            simple_thrust=SIMPLE_THRUST, cg_shift=cg_shift)
...
for i in range(len(real_list_BIRE[:,5])):
    # Phugoid
    if i == 9:
        labela = label1
        labelb = label2
