...
phi = np.deg2rad(20.0) #rad
V = 634 #ft/s
gamma = np.deg2rad(0.0) #rad
H = 15000. #ft
cg_shift = [1., 0., 0.] #ft

climb_angle_array = np.array(np.linspace(0.0,30.0,10))

for i in range(len(climb_angle_array)):

    gamma = np.deg2rad(climb_angle_array[i]) #rad
...
labelF = 'Baseline'
labelB = 'BIRE'
plt.scatter(climb_angle_array, real_list_f16[:,F16_SR_ind], marker=f16_marker, color='k', s = size,label=labelF)
plt.scatter(climb_angle_array, real_list_BIRE[:,BIRE_SR_ind], marker=BIRE_marker, color='k', s = size,label=labelB)
...
labelF = 'Baseline'
labelB = 'BIRE'
plt.scatter(climb_angle_array, real_list_f16[:,F16_RL_ind], marker=f16_marker, color='k', s = size,label=labelF)
plt.scatter(climb_angle_array, real_list_BIRE[:,BIRE_RL_ind], marker=BIRE_marker, color='k', s = size,label=labelB)
