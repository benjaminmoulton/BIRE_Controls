...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 15 14:05:59 2021
...
def generate_data(params):
    ...
    rates = [p, q, r]
    if nasa:
        case.set_state(alpha, beta, d_e, 0., 0., d_a, d_r, 0.35, p, q, r, 0.2, 1000., 0.)
...
def plot_model(x, y, color, ls, tag, marker, ax, label):
    ...
    for i in range(len(y)):
        if tag[i] == "data":
            if first:
                ax.scatter(x, y[i], ec=color[i], fc="None", marker=marker, label=label)
            else:
...
'''THIS SECTION APPEARS TO BE MOSTLY PARSING DATA, maybe applying a mask
to some data outliers, applying some fits, and plotting results.'''
...
def _CL_de(CLde_data, plot, skip_mask=False):
    ...
    CL0, CL_alpha = _CL0_CLalpha(np.array([x for x in CLde_data if x[2] == 0.]), False)
...
def _CL_qbar(CLqbar_data, plot, skip_mask=False):
    ...
    CL0, CL_alpha = _CL0_CLalpha(np.array([x for x in CLqbar_data if x[6] == 0.]), False)
...
def _CS_pbar(CSp_data, plot, skip_mask=False):
    ...
    if plot:
        x = a0
        y = [CSp_p, CSp_m, CS1 + (CS_Lpbar*CL1 + CS_pbar)*pbar, CS1 + (CS_Lpbar*CL1 + CS_pbar)*-pbar]
        color = ['0.0']*4
        ls = [(0, (3, 5, 1, 5, 1, 5))]*4
        tag = ['data', 'data', 'not', 'not']
        marker = '>'
...
def _CD_de(CDde_data, plot, skip_mask=False):
    ...
    CL0, CL_alpha = _CL0_CLalpha(np.array([x for x in CDde_data if x[2] == 0.]), False)
    ...
    if plot:
        x = CL1
        y = [CDde_p, CDde_m, CD1 + (CD_Lde*CL1 + CD_de)*de + CD_de2*de**2, CD1 + (CD_Lde*CL1 + CD_de)*-de + CD_de2*de**2]
        color = ['0.0']*4
        ls = ['--']*4
        tag = ['data', 'data', 'not', 'not']
        marker = 'v'
        return [x, y, color, ls, tag, marker]
    return CD_de, CD_Lde, CD_de2
...
def _CD_pbar(CDp_data, plot, skip_mask=False):
    ...
    CD1 = CDp_p[len(CD1)//2] # Shift to view accuracy of trends rather than discrepancy in CD_pbar
    if plot:
        x = CS1
        y = [CDp_p, CDp_m, CD1 + (CD_Spbar*CS1 + CD_pbar)*pbar, CD1 + (CD_Spbar*CS1 + CD_pbar)*-pbar]
        color = ['0.3']*4
        ls = ['--']*4
        tag = ['data', 'data', 'not', 'not']
        marker = '>'
...
def _CD_rbar(CDr_data, plot, skip_mask=False):
    ...
    CDr_p = np.array([x[8] for x in CDr_data if x[7] == 30.*np.pi/180.])
    CDr_m = np.array([x[8] for x in CDr_data if x[7] == -30.*np.pi/180.])
    rbar = np.deg2rad(30.)*b_w/(2.*V)
    ...
    CD1 = CDr_p[len(CD1)//2] # Shift to view accuracy of trends rather than discrepancy in CD_rbar
    if plot:
        x = CS1
        y = [CDr_p, CDr_m, CD1 + (CD_Srbar*CS1 + CD_rbar)*rbar, CD1 + (CD_Srbar*CS1 + CD_rbar)*-rbar]
        color = ['0.3']*4
        ls = [':']*4
        tag = ['data', 'data', 'not', 'not']
        marker = 's'
...
def _CD_da(CDda_data, plot, skip_mask=False):
    ...
    CD1 = CDda_p[len(CD1)//2] # Shift to view accuracy of trends rather than discrepancy in CD_da
    if plot:
        x = CS1
        y = [CDda_p, CDda_m, CD1 + (CD_Sda*CS1 + CD_da)*da, CD1 + (CD_Sda*CS1 + CD_da)*-da]
        color = ['0.3']*4
        ls = ['-.']*4
        tag = ['data', 'data', 'not', 'not']
        marker = 'h'
...
def _CD_dr(CDdr_data, plot, skip_mask=False):
    ...
    CD_0, CD_S, CD_S2 = _CD_Spolar(np.array([x for x in CDdr_data if x[4] == 0.]), False)
    CD1 = CDdr_p[len(CD1)//2] # Shift to view accuracy of trends rather than discrepancy in CD_dr
    if plot:
        x = CS1
        y = [CDdr_p, CDdr_m, CD1 + (CD_Sdr*CS1 + CD_dr)*dr, CD1 + (CD_Sdr*CS1 + CD_dr)*-dr]
        color = ['0.3']*4
        ls = [(0, (3, 5, 1, 5, 1, 5))]*4
        tag = ['data', 'data', 'not', 'not']
        marker = 'd'
...
def _CD_qbar(CDq_data, plot, skip_mask=False):
    ...
    alpha = a0/180.*np.pi
    CL = CL0 + CL_alpha*alpha
    CD_0, CD_L, CD_L2 = _CD_polar(np.array([x for x in CDq_data if x[6] == 0.]), False)
    CD1 = CD_0 + CD_L*CL + CD_L2*np.square(CL)
    if plot:
        x = CL1
        y = [CDq_p, CDq_m, CD1 + (CD_L2qbar*np.square(CL1) + CD_Lqbar*CL1 + CD_qbar)*qbar, CD1 + (CD_L2qbar*np.square(CL1) + CD_Lqbar*CL1 + CD_qbar)*-qbar]
        color = ['0.0']*4
        ls = [':']*4
        tag = ['data', 'data', 'not', 'not']
        marker = '^'
...
def _Cl_rbar(Clr_data, plot, skip_mask=False):
    ...
    Clr_p = np.array([x[11] for x in Clr_data if x[7] == 30.*np.pi/180.])
    Clr_m = np.array([x[11] for x in Clr_data if x[7] == -30.*np.pi/180.])
    rbar = np.deg2rad(30.)*b_w/(2.*V)
    Cl_rbar = (Clr_p - Clr_m)/(2.*rbar)
    if skip_mask:
        out_mask = [True]*len(CL1)
    else:
        out_mask = remove_outliers(Cl_rbar)
    [Cl_Lrbar, Cl_rbar] = np.polyfit(CL1[out_mask], Cl_rbar[out_mask], 1)

    a0 = np.array([x[0] for x in Clr_data if x[7] == 0.])
    Cl1 = np.zeros(len(a0))
    [CL0, CL_alpha] = _CL0_CLalpha(np.array([x for x in Clr_data if x[7] == 0.]), False)
    CL1 = CL0 + CL_alpha*a0*np.pi/180.
    if plot:
        x = a0
        y = [Clr_p, Clr_m, Cl1 + (Cl_Lrbar*CL1 + Cl_rbar)*rbar, Cl1 + (Cl_Lrbar*CL1 + Cl_rbar)*-rbar]
        color = ['0.0']*4
        ls = [(0, (3, 5, 1, 5, 1, 5))]*4
        tag = ['data', 'data', 'not', 'not']
        marker = '>'
...
def _Cm_qbar(Cmq_data, plot, skip_mask=False):
    ...
    [Cm0, Cm_alpha] = _Cm0_Cmalpha(np.array([x for x in Cmq_data if x[6] == 0.]), False)
...
def _Cm_de(Cmde_data, plot, skip_mask=False):
    ...
    [Cm0, Cm_alpha] = _Cm0_Cmalpha(np.array([x for x in Cmde_data if x[2] == 0.]), False)
    Cm1 = Cm0 + Cm_alpha*a0*np.pi/180.
    if plot:
        x = a0
...
def _Cn_pbar(Cnp_data, plot, skip_mask=False):
    ...
    if plot:
        x = a0
        y = [Cnp_p, Cnp_m, Cn1 + (Cn_Lpbar*CL1 + Cn_pbar)*pbar, Cn1 + (Cn_Lpbar*CL1 + Cn_pbar)*-pbar]
        color = ['0.0']*4
        ls = [':']*4
        tag = ['data', 'data', 'not', 'not']
        marker = '^'
...
def find_model(database):
    '''pulls data from the database and solves for coefficients?'''
    plot = False
    df = pd.DataFrame(database, columns = ['Alpha','Beta','d_e', 'd_a', 'd_r', 'p', 'q', 'r', 'CD', 'CS', 'CL', 'Cl', 'Cm', 'Cn'])
    CLalpha_data = df.loc[df['Beta'] + df['d_e'] + df['d_a'] + df['d_r'] + df['p'] + df['q'] + df['r'] == 0].to_numpy()
    CL_0, CL_alpha = _CL0_CLalpha(CLalpha_data, plot)
    CLde_data = df.loc[df['Beta'] + df['d_a'] + df['d_r'] + df['p'] + df['q'] + df['r'] == 0].to_numpy()
    CL_de = _CL_de(CLde_data, plot)
    CLqbar_data = df.loc[df['Beta'] + df['d_e'] + df['d_a'] + df['d_r'] + df['p'] + df['r'] == 0].to_numpy()
    CL_qbar = _CL_qbar(CLqbar_data, plot)
    CSbeta_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_a'] + df['d_r'] + df['p'] + df['q'] + df['r'] == 0) & (df['Alpha'] == 0.))].sort_values(by=['Beta']).to_numpy()
    CS_0, CS_beta = _CS_beta(CSbeta_data, plot)
    CSda_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_r'] + df['p'] + df['q'] + df['r'] == 0) & (df['Alpha'] == 0.))].sort_values(by=['Beta']).to_numpy()
    CS_da = _CS_da(CSda_data, plot)
    CSdr_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_a'] + df['p'] + df['q'] + df['r'] == 0) & (df['Alpha'] == 0.))].sort_values(by=['Beta']).to_numpy()
    CS_dr = _CS_dr(CSdr_data, plot)
    CSr_data = df.loc[((df['Beta'] + df['d_e'] + df['d_a'] + df['p'] + df['q'] + df['d_r'] == 0))].to_numpy()
    CS_rbar = _CS_rbar(CSr_data, plot)
    CSp_data = df.loc[((df['Beta'] + df['d_e'] + df['d_a'] + df['d_r'] + df['q'] + df['r'] == 0))].to_numpy()
    CS_pbar, CS_Lpbar = _CS_pbar(CSp_data, plot, skip_mask=True)
    CDde_data = df.loc[((df['Beta'] + df['p'] + df['d_a'] + df['d_r'] + df['q'] + df['r'] == 0))].to_numpy()
    ...
    CDp_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_a'] + df['d_r'] + df['q'] + df['r'] == 0) & (df['Alpha'] == 0.))].to_numpy()
    CDr_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_a'] + df['d_r'] + df['q'] + df['p'] == 0) & (df['Alpha'] == 0.))].to_numpy()
    ...
    Cnda_data = df.loc[((df['Beta'] + df['p'] + df['d_e'] + df['d_r'] + df['q'] + df['r'] == 0))].to_numpy()
...
def create_plots(M, N, save=False):
    plot=True
    df = pd.DataFrame(M, columns = ['Alpha','Beta','d_e', 'd_a', 'd_r', 'p', 'q', 'r', 'CD', 'CS', 'CL', 'Cl', 'Cm', 'Cn'])

    fig_CL, ax_CL = plt.subplots()
    CLalpha_data = df.loc[df['Beta'] + df['d_e'] + df['d_a'] + df['d_r'] + df['p'] + df['q'] + df['r'] == 0].to_numpy()
    fig_data = _CL0_CLalpha(CLalpha_data, plot)
    plot_model(*fig_data, ax=ax_CL, label='Nominal')

    CLde_data = df.loc[df['Beta'] + df['d_a'] + df['d_r'] + df['p'] + df['q'] + df['r'] == 0].to_numpy()
    fig_data = _CL_de(CLde_data, plot)
    plot_model(*fig_data, ax=ax_CL, label=r'$\pm \Delta \delta_e$')

    CLqbar_data = df.loc[df['Beta'] + df['d_e'] + df['d_a'] + df['d_r'] + df['p'] + df['r'] == 0].to_numpy()
    fig_data = _CL_qbar(CLqbar_data, plot)
    plot_model(*fig_data, ax=ax_CL, label=r'$\pm \Delta \overline{q}$')
    xlims = (-12, 12)
    ylims = (-1, 1)
    dx = {"major": 5, "minor": 1}
    dy = {"major": 0.2, "minor": 0.05}
    axs = pretty_plot(ax_CL, xlims, ylims, dx, dy)
    axs.grid()
    ax_CL.set_xlabel(r'\textbf{Angle of Attack, }\boldmath$\alpha$\textbf{ [deg]}', fontsize=14)
    ax_CL.set_ylabel(r'\textbf{Lift Coefficient, }\boldmath$C_L$', fontsize=14)
    ax_CL.legend(fontsize=14)
    plt.tight_layout()
    if save:
        plt.savefig('./Aerodynamic Model Figures/CL_Baseline.pdf', dpi=1000)

    fig_CS, ax_CS = plt.subplots(2, 1)
    CSbeta_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_a'] + df['d_r'] + df['p'] + df['q'] + df['r'] == 0) & (df['Alpha'] == 0.))].sort_values(by=['Beta']).to_numpy()
    fig_data = _CS_beta(CSbeta_data, plot)
    plot_model(*fig_data, ax=ax_CS[0], label='Nominal')

    CSda_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_r'] + df['p'] + df['q'] + df['r'] == 0) & (df['Alpha'] == 0.))].sort_values(by=['Beta']).to_numpy()
    fig_data = _CS_da(CSda_data, plot)
    plot_model(*fig_data, ax=ax_CS[0], label=r'$\Delta \delta_a$')

    CSdr_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_a'] + df['p'] + df['q'] + df['r'] == 0) & (df['Alpha'] == 0.))].sort_values(by=['Beta']).to_numpy()
    fig_data = _CS_dr(CSdr_data, plot)
    plot_model(*fig_data, ax=ax_CS[0], label=r'$\Delta \delta_r$')
    xlims = (-7, 7)
    ylims = (-0.35, 0.35)
    dx = {"major": 2, "minor": 0.5}
    dy = {"major": 0.2, "minor": 0.05}
    axs = pretty_plot(ax_CS[0], xlims, ylims, dx, dy)
    axs.grid()
    ax_CS[0].set_xlabel(r'\textbf{Sideslip Angle, }\boldmath$\beta$\textbf{ [deg]}', fontsize=14)
    fig_CS.supylabel(r'\textbf{Side Force Coefficient, }\boldmath$C_S$', fontsize=14)
    ax_CS[0].legend(fontsize=14)

    CSr_data = df.loc[((df['Beta'] + df['d_e'] + df['d_a'] + df['p'] + df['q'] + df['d_r'] == 0))].to_numpy()
    fig_data = _CS_rbar(CSr_data, plot)
    plot_model(*fig_data, ax=ax_CS[1], label=r'$\pm \Delta \overline{r}$')

    CSp_data = df.loc[((df['Beta'] + df['d_e'] + df['d_a'] + df['d_r'] + df['q'] + df['r'] == 0))].to_numpy()
    fig_data = _CS_pbar(CSp_data, plot)
    plot_model(*fig_data, ax=ax_CS[1], label=r'$\pm \Delta \overline{p}$')
    xlims = (-12, 12)
    ylims = (-0.07, 0.07)
    dx = {"major": 5, "minor": 1}
    dy = {"major": 0.03, "minor": 0.0075}
    axs = pretty_plot(ax_CS[1], xlims, ylims, dx, dy)
    axs.grid()
    ax_CS[1].set_xlabel(r'\textbf{Angle of Attack, }\boldmath$\alpha$\textbf{ [deg]}', fontsize=14)
    ax_CS[1].legend(fontsize=14)
    plt.tight_layout()
    if save:
        plt.savefig('./Aerodynamic Model Figures/CS_Baseline.pdf', dpi=1000)

    fig_CD, ax_CD = plt.subplots(2, 1)
    fig_data = _CD_polar(CLalpha_data, plot)
    plot_model(*fig_data[:-1], ax=ax_CD[0], label=r'Polar')

    fig_data = _CD_qbar(CLqbar_data, plot)
    plot_model(*fig_data, ax=ax_CD[0], label=r'$\pm \Delta \overline{q}$')
    ax_CD[0].annotate(r'\boldmath$+ \Delta \overline{q}$', (-0.6, 0.015), fontsize=14, color='0.5')

    CDde_data = df.loc[((df['Beta'] + df['p'] + df['d_a'] + df['d_r'] + df['q'] + df['r'] == 0))].to_numpy()
    fig_data = _CD_de(CDde_data, plot)
    plot_model(*fig_data, ax=ax_CD[0], label=r'$\pm \Delta \delta_e$')
    ax_CD[0].annotate(r'\boldmath$+ \Delta \delta_e$', (-0.5, 0.09), fontsize=14, color='0.5')
    xlims = (-0.7, 0.8)
    ylims = (-0.01, 0.11)
    dx = {"major": 0.2, "minor": 0.05}
    dy = {"major": 0.03, "minor": 0.0075}
    axs = pretty_plot(ax_CD[0], xlims, ylims, dx, dy)
    axs.grid()
    ax_CD[0].set_xlabel(r'\textbf{Lift Coefficient, }\boldmath$C_L$', fontsize=14)
    fig_CD.supylabel(r'\textbf{Drag Coefficient, }\boldmath$C_D$', fontsize=14)
    ax_CD[0].legend(fontsize=14, ncol=3, loc='upper center')

    fig_data = _CD_Spolar(CSbeta_data, plot)
    plot_model(*fig_data[:-1], ax=ax_CD[1], label=r'Polar')

    CDp_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_a'] + df['d_r'] + df['q'] + df['r'] == 0) & (df['Alpha'] == 0.))].to_numpy()
    CDr_data = df.loc[((df['Alpha'] + df['d_e'] + df['d_a'] + df['d_r'] + df['q'] + df['p'] == 0) & (df['Alpha'] == 0.))].to_numpy()
    fig_data = _CD_pbar(CDp_data, plot)
    plot_model(*fig_data, ax=ax_CD[1], label=r'$\pm \Delta \overline{p}$')

    fig_data = _CD_rbar(CDr_data, plot)
    plot_model(*fig_data, ax=ax_CD[1], label=r'$\pm \Delta \overline{r}$')
    xlims = (-0.09, 0.09)
    ylims = (0.0005, 0.085)
    dx = {"major": 0.04, "minor": 0.01}
    dy = {"major": 0.02, "minor": 0.02/4}
    axs = pretty_plot(ax_CD[1], xlims, ylims, dx, dy)
    axs.grid()
    ax_CD[1].set_xlabel(r'\textbf{Side Force Coefficient, }\boldmath$C_S$', fontsize=14)

    fig_data = _CD_da(CSda_data, plot)
    plot_model(*fig_data, ax=ax_CD[1], label=r'$\pm \Delta \delta_a$')

    fig_data = _CD_dr(CSdr_data, plot)
    plot_model(*fig_data, ax=ax_CD[1], label=r'$\pm \Delta \delta_r$')
    ax_CD[1].legend(fontsize=14, ncol=3, loc='upper center')
    plt.tight_layout()
    if save:
        plt.savefig('./Aerodynamic Model Figures/CD_Baseline.pdf', dpi=1000)

    fig_Cl, ax_Cl = plt.subplots(2, 1)
    fig_data = _Cl_beta(CSbeta_data, plot)
    plot_model(*fig_data, ax=ax_Cl[0], label=r'Nominal')

    fig_data = _Cl_da(CSda_data, plot)
    plot_model(*fig_data, ax=ax_Cl[0], label=r'$\Delta \delta_a$')

    fig_data = _Cl_dr(CSdr_data, plot)
    plot_model(*fig_data, ax=ax_Cl[0], label=r'$\Delta \delta_r$')
    xlims = (-7, 7)
    ylims = (-0.05, 0.05)
    dx = {"major": 2, "minor": 0.5}
    dy = {"major": 0.02, "minor": 0.005}
    axs = pretty_plot(ax_Cl[0], xlims, ylims, dx, dy)
    axs.grid()
    ax_Cl[0].set_xlabel(r'\textbf{Sideslip Angle, }\boldmath$\beta$\textbf{, [deg]}', fontsize=14)
    fig_Cl.supylabel(r'\textbf{Rolling Moment Coefficient, }\boldmath$C_\ell$', fontsize=14)
    ax_Cl[0].legend(fontsize=14, loc='upper right')

    fig_data = _Cl_pbar(CSp_data, plot)
    plot_model(*fig_data, ax=ax_Cl[1], label=r'$\pm \Delta \overline{p}$')

    fig_data = _Cl_rbar(CSr_data, plot)
    plot_model(*fig_data, ax=ax_Cl[1], label=r'$\pm \Delta \overline{r}$')
    xlims = (-12, 12)
    ylims = (-0.05, 0.05)
    dx = {"major": 5, "minor": 1}
    dy = {"major": 0.02, "minor": 0.005}
    axs = pretty_plot(ax_Cl[1], xlims, ylims, dx, dy)
    axs.grid()
    ax_Cl[1].set_xlabel(r'\textbf{Angle of Attack, }\boldmath$\alpha$\textbf{, [deg]}', fontsize=14)
    ax_Cl[1].legend(fontsize=14, loc='lower right')
    plt.tight_layout()
    if save:
        plt.savefig('./Aerodynamic Model Figures/Cl_Baseline.pdf', dpi=1000)

    fig_Cm, ax_Cm = plt.subplots()
    fig_data = _Cm0_Cmalpha(CLalpha_data, plot)
    plot_model(*fig_data, ax=ax_Cm, label=r'Nominal')

    fig_data = _Cm_qbar(CLqbar_data, plot)
    plot_model(*fig_data, ax=ax_Cm, label=r'$\pm \Delta \overline{q}$')

    fig_data = _Cm_de(CLde_data, plot)
    plot_model(*fig_data, ax=ax_Cm, label=r'$\pm \Delta \delta_e$')
    xlims = (-12, 12)
    ylims = (-0.35, 0.35)
    dx = {"major": 5, "minor": 1}
    dy = {"major": 0.1, "minor": 0.025}
    axs = pretty_plot(ax_Cm, xlims, ylims, dx, dy)
    axs.grid()
    ax_Cm.set_xlabel(r'\textbf{Angle of Attack, }\boldmath$\alpha$\textbf{, [deg]}', fontsize=14)
    ax_Cm.set_ylabel(r'\textbf{Pitching Moment Coefficient, }\boldmath$C_m$')
    ax_Cm.legend(fontsize=14)
    plt.tight_layout()
    if save:
        plt.savefig('./Aerodynamic Model Figures/Cm_Baseline.pdf', dpi=1000)


    fig_Cn, ax_Cn = plt.subplots(2, 1)
    fig_data = _Cn_beta(CSbeta_data, plot)
    plot_model(*fig_data, ax=ax_Cn[0], label=r'Nominal')

    fig_data = _Cn_pbar(CSp_data, plot)
    plot_model(*fig_data, ax=ax_Cn[1], label=r'$\Delta \pm \overline{p}$')

    fig_data = _Cn_rbar(CSr_data, plot)
    plot_model(*fig_data, ax=ax_Cn[1], label=r'$\Delta \pm \overline{r}$')

    Cnda_data = df.loc[((df['Beta'] + df['p'] + df['d_e'] + df['d_r'] + df['q'] + df['r'] == 0))].to_numpy()
    fig_data = _Cn_da(Cnda_data, plot)
    plot_model(*fig_data, ax=ax_Cn[1], label=r'$\Delta \delta_a$')
    xlims = (-12, 12)
    ylims = (-0.035, 0.035)
    dx = {"major": 5, "minor": 1}
    dy = {"major": 0.01, "minor": 0.0025}
    axs = pretty_plot(ax_Cn[1], xlims, ylims, dx, dy)
    axs.grid()
    ax_Cn[1].set_xlabel(r'\textbf{Angle of Attack, }\boldmath$\alpha$\textbf{, [deg]}', fontsize=14)
    ax_Cn[1].legend(fontsize=14)

    fig_data = _Cn_dr(CSdr_data, plot)
    plot_model(*fig_data, ax=ax_Cn[0], label=r'$\Delta \delta_r$')
    xlims = (-7, 7)
    ylims = (-0.1, 0.05)
    dx = {"major": 2, "minor": 0.5}
    dy = {"major": 0.02, "minor": 0.005}
    axs = pretty_plot(ax_Cn[0], xlims, ylims, dx, dy)
    axs.grid()
    ax_Cn[0].set_xlabel(r'\textbf{Sideslip Angle, }\boldmath$\beta$\textbf{, [deg]}', fontsize=14)
    fig_Cn.supylabel(r'\textbf{Yawing Moment Coefficient, }\boldmath$C_n$', fontsize=14)
    ax_Cn[0].legend(fontsize=14)
    plt.tight_layout()
    if save:
        plt.savefig('./Aerodynamic Model Figures/Cn_Baseline.pdf', dpi=1000)
...
if __name__ == "__main__":
    ...
    Nfile_exists = exists(path_to_Ndb_file) # check if the NASA database file exists
    Mfile_exists = exists(path_to_Mdb_file) # check if the F16 MU database file exists
    ...
    if generate_plots:
        M = create_plots(M_database, N_database, save)
