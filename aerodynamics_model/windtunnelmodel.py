# -*- coding: utf-8 -*-
"""
Created on Tue May 26 14:39:16 2020
...
class F16_windtunnel:
    def __init__(self, data_dir="./NASA Data/Python Data/"):
        ...
        self.CXq_a = interp1d(alpha, C_Xq, bounds_error=False, fill_value="extrapolate")
        DC_Xsb = np.load(data_dir + "DC_X,sb(a).npy")
        self.DCXsb_a = interp1d(alpha, DC_Xsb, bounds_error=False, fill_value="extrapolate")
        DC_Xqlef = np.load(data_dir + "DC_X_q,lef(a).npy")
        self.DCXqlef_a = interp1d(alpha_lef, DC_Xqlef, bounds_error=False, fill_value="extrapolate")
        ...
        self.CYp_a = interp1d(alpha, C_Yp, bounds_error=False, fill_value="extrapolate")
        C_Yr = np.load(data_dir + "C_Y_r(a).npy")
        self.CYr_a = interp1d(alpha, C_Yr, bounds_error=False, fill_value="extrapolate")
        DC_Yplef = np.load(data_dir + "DC_Y_p,lef(a).npy")
        self.DCYplef_a = interp1d(alpha_lef, DC_Yplef, bounds_error=False, fill_value="extrapolate")
        DC_Yrlef = np.load(data_dir + "DC_Y_r,lef(a).npy")
        self.DCYrlef_a = interp1d(alpha_lef, DC_Yrlef, bounds_error=False, fill_value="extrapolate")
        ...
        self.CZq_a = interp1d(alpha, C_Zq, bounds_error=False, fill_value="extrapolate")
        DC_Zsb = np.load(data_dir + "DC_Z,sb(a).npy")
        self.DCZsb_a = interp1d(alpha, DC_Zsb, bounds_error=False, fill_value="extrapolate")
        DC_Zqlef = np.load(data_dir + "DC_Z_q,lef(a).npy")
        self.DCZqlef_a = interp1d(alpha_lef, DC_Zqlef, bounds_error=False, fill_value="extrapolate")
        ...
        self.Cmq_a = interp1d(alpha, C_mq, bounds_error=False, fill_value="extrapolate")
        DC_m = np.load(data_dir + "DC_m(a).npy")
        self.DCm_a = interp1d(alpha, DC_m, bounds_error=False, fill_value="extrapolate")
        ...
        self.DCmsb_a = interp1d(alpha, DC_msb, bounds_error=False, fill_value="extrapolate")
        DC_mqlef = np.load(data_dir + "DC_m_q,lef(a).npy")
        self.DCmqlef_a = interp1d(alpha_lef, DC_mqlef, bounds_error=False, fill_value="extrapolate")
        n_dh = np.load(data_dir + "n_d_h(d_h).npy")
        self.ndh_dh = interp1d(dh, n_dh, bounds_error=False, fill_value="extrapolate")
        ...
        self.Clp_a = interp1d(alpha, C_lp, bounds_error=False, fill_value="extrapolate")
        C_lr = np.load(data_dir + "C_l_r(a).npy")
        self.Clr_a = interp1d(alpha, C_lr, bounds_error=False, fill_value="extrapolate")
        DC_lb = np.load(data_dir + "DC_l_b(a).npy")
        self.DClb_a = interp1d(alpha, DC_lb, bounds_error=False, fill_value="extrapolate")
        DC_lplef = np.load(data_dir + "DC_l_p,lef(a).npy")
        self.DClplef_a = interp1d(alpha_lef, DC_lplef, bounds_error=False, fill_value="extrapolate")
        DC_lrlef = np.load(data_dir + "DC_l_r,lef(a).npy")
        self.DClrlef_a = interp1d(alpha_lef, DC_lrlef, bounds_error=False, fill_value="extrapolate")
        ...
        self.Cnp_a = interp1d(alpha, C_np, bounds_error=False, fill_value="extrapolate")
        C_nr = np.load(data_dir + "C_n_r(a).npy")
        self.Cnr_a = interp1d(alpha, C_nr, bounds_error=False, fill_value="extrapolate")
        DC_nb = np.load(data_dir + "DC_n_b(a).npy")
        self.DCnb_a = interp1d(alpha, DC_nb, bounds_error=False, fill_value="extrapolate")
        DC_nda = np.load(data_dir + "DC_n_d_a(a).npy")
        self.DCnda_a = interp1d(alpha, DC_nda, bounds_error=False, fill_value="extrapolate")
        DC_nplef = np.load(data_dir + "DC_n_p,lef(a).npy")
        self.DCnplef_a = interp1d(alpha_lef, DC_nplef, bounds_error=False, fill_value="extrapolate")
        DC_nrlef = np.load(data_dir + "DC_n_r,lef(a).npy")
        self.DCnrlef_a = interp1d(alpha_lef, DC_nrlef, bounds_error=False, fill_value="extrapolate")
    ...
    def thrust(self):
        ...
        if pow_c >= 50.0:
            T = self.T_mil(MH) + (self.T_max(MH) - self.T_mil(MH))*(pow_c - 50.0)/50.0
        else:
    ...
    def calc_forces(self, body_frame=True, stab_frame=False,
        ...
        if body_frame:
            ...
            if dimensional:
                ...
                forces['F16'].update({key : fm for key, fm in zip(body_keys_dim, body_fm_dim)})
        if stab_frame:
            ...
            if dimensional:
                ...
                forces['F16'].update({key : fm for key, fm in zip(stab_keys_dim, stab_fm_dim)})
        if wind_frame:
            ...
            if dimensional:
                ...
                forces['F16'].update({key : fm for key, fm in zip(wind_keys_dim, wind_fm_dim)})
        if verbose:
            print(forces['F16'])
        return forces
